...
TEST_CASE("Construct queue") { dp::work_stealing_deque<std::uint64_t> queue{}; }

TEST_CASE("Construct and grow queue") {
...
TEST_CASE("Take bottom while queue is empty") {
    dp::work_stealing_deque<std::uint64_t> queue{};

    REQUIRE_EQ(queue.take_bottom(), std::nullopt);
}

TEST_CASE("Take bottom while queue is not empty") {
    dp::work_stealing_deque<std::uint64_t> queue{};
...
TEST_CASE("Multiple thread steal single item") {
    dp::work_stealing_deque<std::uint64_t> queue{};
...
TEST_CASE("Steal std::function while pushing") {
...
//     move_only(move_only&& other) noexcept { private_value_ = other.private_value_ * 2; }
//     move_only& operator=(move_only&) = delete;
//     move_only& operator=(move_only&& other) noexcept {
//         private_value_ = other.private_value_ * 2;
//         return *this;
//     }
//     [[nodiscard]] int secret() const { return private_value_; }
// };
//
// TEST_CASE("Store move only types") {
//     move_only mv_only{};
...
// }
//
// TEST_CASE("Steal move only type") {
//     move_only mv_only{};
...
//
// TEST_CASE("Steal std::move_only_function while pushing") {
